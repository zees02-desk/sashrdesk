...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // sashrdesk://<connect-id>
    // sashrdesk://<connect-id>/r
    // sashrdesk://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing SASHRDesk...");
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or sashrdesk-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.sash.sashrdesk/macos");
...
Widget loadPowered(BuildContext context) {
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://sash.com.my'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
            ...
            children: [
              Align(
                ...
                style: TextStyle(
                  color: Colors.red,
                  fontSize:
                      18, // https://github.com/Zees02/rustdesk-server-pro/issues/261
                  fontWeight: FontWeight.bold,
                ),
