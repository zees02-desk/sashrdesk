...
class PlatformFFI {
  ...
  SASHRDeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libsashrdesk.so')
        : isLinux
            ? DynamicLibrary.open('libsashrdesk.so')
            : isWindows
                ? DynamicLibrary.open('libsashrdesk.dll')
                : isMacOS
                    ? DynamicLibrary.open("liblibsashrdesk.dylib")
  ...
  void _startListenEvent(SASHRDeskImpl sashrdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = sashrdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
