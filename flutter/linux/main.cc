...
#define RUSTDESK_LIB_PATH "libsashrdesk.so"
...
bool flutter_rustdesk_core_main() {
   void* libsashrdesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libsashrdesk) {
      fprintf(stderr,"Failed to load \"libsashrdesk.so\"\n");
      char* error;
   ...
   auto core_main = (RustDeskCoreMain) dlsym(libsashrdesk,"rustdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
