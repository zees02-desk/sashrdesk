...
{
  HINSTANCE hInstance = LoadLibraryA("libsashrdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libsashrdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!c_args)
  {
    ...
    // std::cout << "SASHRDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"SASHRDesk";
