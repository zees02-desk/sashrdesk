...
fn get_after_install(exe: &str) -> String {
    ...
    // https://github.com/Zees02/rustdesk/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/Zees02/rustdesk/issues/895
    let dels = format!(
        "
...
fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    // https://github.com/Zees02/rustdesk/issues/6786#issuecomment-1879655410
...
pub fn try_kill_rustdesk_main_window_process() -> ResultType<()> {
    // Kill sashrdesk.exe without extra arg, should only be called by --server
    // We can find the exact process which occupies the ipc, see more from https://github.com/winsiderss/systeminformer
    log::info!("try kill sashrdesk main window process");
    ...
    bail!("failed to find sashrdesk main window process");
}
