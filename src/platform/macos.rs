...
// remove just one app from all the permissions: tccutil reset All com.sash.sashrdesk
...
fn correct_app_name(s: &str) -> String {
    let s = s.replace("sashrdesk", &crate::get_app_name().to_lowercase());
    let s = s.replace("SASHRDesk", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/SASHRDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another SASHRDesk.app
