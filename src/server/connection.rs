...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            hash,
            read_jobs: Vec::new(),
            timer: crate::sashrdesk_interval(time::interval(SEC30)),
            file_timer: crate::sashrdesk_interval(time::interval(SEC30)),
            file_transfer: None,
        ...
        let mut test_delay_timer =
            crate::sashrdesk_interval(time::interval_at(Instant::now(), TEST_DELAY_TIMEOUT));
        ...
        let mut second_timer = crate::sashrdesk_interval(time::interval(Duration::from_secs(1)));
            ...
            tokio::select! {
                ...
                _ = conn.file_timer.tick() => {
                    if !conn.read_jobs.is_empty() {
                        ...
                        conn.file_timer = crate::sashrdesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
                    ...
                    if self.file_transfer.is_some() {
                        ...
                        match fa.union {
                            ...
                            Some(file_action::Union::Send(s)) => {
                                ...
                                match fs::TransferJob::new_read(
                                    ...
                                    Ok(mut job) => {
                                        ...
                                        self.file_timer =
                                            crate::sashrdesk_interval(time::interval(MILLI1));
                            ...
                            Some(file_action::Union::Receive(r)) => {
                                ...
                                // whenever got send/recv request, check peer version to ensure old version of sashrdesk
                                let od = can_enable_overwrite_detection(get_version_number(
                                    &self.lr.version,
                                ));
        ...
        if self.keyboard {
            if let Ok(displays) = display_service::try_get_displays() {
                ...
                if let Some(display) = displays.get(display_idx) {
                    ...
                    #[cfg(windows)]
                    if let Some(_ok) =
                        virtual_display_manager::sashrdesk_idd::change_resolution_if_is_virtual_display(
                            &name,
                            r.width as _,
                            r.height as _,
                        )
