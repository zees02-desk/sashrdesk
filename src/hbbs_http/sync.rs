...
async fn start_hbbs_sync_async() {
    let mut interval = crate::sashrdesk_interval(tokio::time::interval_at(
        ...
        tokio::select! {
            _ = interval.tick() => {
                ...
                if !info_uploaded.0 && info_uploaded.2.map(|x| x.elapsed() >= UPLOAD_SYSINFO_TIMEOUT).unwrap_or(true) {
                    ...
                    // real user name not uploaded after login screen. https://github.com/Zees02/rustdesk/discussions/8031
