...
pub fn video_save_directory(root: bool) -> String {
    ...
    if root {
        ...
        #[cfg(windows)]
        {
            ...
            let dir =
                std::path::PathBuf::from(format!("{drive}\\ProgramData\\SASHRDesk\\recording",));
    ...
    if let Ok(home) = config::APP_HOME_DIR.read() {
        ...
        path.push_str("/SASHRDesk/ScreenRecord");
...
async fn check_connect_status_(reconnect: bool, rx: mpsc::UnboundedReceiver<ipc::Data>) {
    ...
    loop {
        if let Ok(mut c) = ipc::connect(1000, "").await {
            let mut timer = crate::sashrdesk_interval(time::interval(time::Duration::from_secs(1)));
            loop {
                tokio::select! {
                    res = c.next() => {
                        match res {
